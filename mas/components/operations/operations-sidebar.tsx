...
import { Activity, BarChart3, Bot, Boxes, ChevronLeft, ChevronRight, ClipboardCheck, ClipboardList, FileSearch, Gauge, Home, PackageCheck, Settings, ShoppingBag, Users, Wrench, BriefcaseBusiness } from "lucide-react";
...
export function OperationsSidebar({
  ...
  return (
    <aside className={`sticky top-0 hidden h-screen shrink-0 border-r border-white/10 bg-[#060a12]/95 p-4 text-white transition-[width] duration-200 lg:block ${isCollapsed ? "w-20" : "w-72"}`}>
      <div className={`mb-5 flex items-center gap-3 ${isCollapsed ? "justify-center" : "justify-between"}`}>
        ...
        <button
          ...
          className="hidden h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/[0.06] text-slate-300 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-300 lg:flex"
